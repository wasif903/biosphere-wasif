...
import {
  ...
  usePostRatingsAndReviewsMutation,
...
function getStars(ratings, maxStars = 5) {
  // Ensure ratings is a valid number
  ratings = Number(ratings);

  // Clamp ratings to be between 1 and maxStars
  if (isNaN(ratings) || ratings < 1) ratings = 1;
  if (ratings > maxStars) ratings = maxStars;
...
const Page = () => {
  ...
  const userID = getCookieData?._id;
  ...
  const ProductId = data_?._id || "";
  const storeID = data_?.storeID?._id || "";
  ...
  //   post review api
  const [postReview, { isLoading }] = usePostRatingsAndReviewsMutation();
      ...
      <div className="w-full grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="grid grid-cols-1 gap-5 max-lg:p-8">
          <div className="rounded-lg h-fit p-0 max-sm:p-5">
            <h1 className="text-primary-color "> {data_?.title} </h1>
            <h4
              className="text-primary-color text-base font-medium mt-5"
              dangerouslySetInnerHTML={{ __html: data_?.desc }}
            ></h4>
          </div>
      ...
      <div className="w-full max-lg:p-8">
        ...
        <div className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 py-[1.5rem]">
              ...
              {[1, 2, 3, 4, 5].map((value) => (
                <FaStar
                  ...
                  className={`text-2xl cursor-pointer ${
                    value <= rating ? "text-yellow-400" : "text-[#E8E8E8]"
                  }`}
          ...
          <div className="bg-white box_shadow_pri p-7 max-h-fit flex flex-col justify--center items-center rounded-xl">
            ...
            {data_?.ratings?.map((item, i) => {
              const stars = getStars(item?.ratings);
              return (
                <div className="border-b border-[#c0c0c056] w-full mt-2 flex flex-col items-start gap-2 pb-3">
                  <div className="w-full flex gap-3">
                    <img
                      src={item?.userID?.profile_image || "/images/pic.png"}
                      alt="img"
