...
import { useGetSingleProductQuery, useUpdateProductMutation } from "@/redux/ProductSlice/productslice";
...
function page() {
    ...
    // Handle Remove Category from Array
    const handleRemoveCategory = (id) => {
        if (category?.some(c => c.createdAt)) {
            setProductData((prevState) => ({
                ...prevState,
                category: prevState.category?.filter((item) => id !== item?._id)
            }))
        } else {
            setProductData((prevState) => ({
                ...prevState,
                category: prevState.category?.filter((item) => id !== item)
            }))
    ...
    const handleUpdateProduct = async () => {
        try {
            ...
            if (isVariableCondition === false) {
                ...
                formData.append("slut", params?.slug)
                if (category.some(item => typeof item === 'object' && item.createdAt)) {
                    const categoryIds = category.map(cat => (typeof cat === 'object' ? cat._id : cat));
            ...
            const res = await updateProductAPI({ storeID: storeID, data: formData })
            ...
            ResponseToast({ message: "Error Updating product" })
        }
    }
    ...
    const getSingleProduct = useGetSingleProductQuery({
        storeID: storeID,
        prodID: params?.slug
    })
    const getSingleProductData = getSingleProduct?.data

    // Set Product Data in state
    useEffect(() => {
        setProductData({
            title: getSingleProductData?.title,
            productImage: getSingleProductData?.productImage,
            galleryImages: getSingleProductData?.galleryImages,
            desc: getSingleProductData?.desc,
            slug: getSingleProductData?.slug,
            price: getSingleProductData?.price,
            stock: getSingleProductData?.stock,
            isVariable: getSingleProductData?.isVariable,
            variations: getSingleProductData?.variations.map((item) => ({
                price: item?.price,
                minQty: item?.minQty,
                maxQty: item?.maxQty,
            })),
            category: getSingleProductData?.category
        })
        setIsVariableCondition(getSingleProductData?.isVariable)
        setAddVariableToggler(getSingleProductData?.isVariable)
        setIsCategory(getSingleProductData?.category?.length <= 0 ? false : true)
    }, [getSingleProduct])
                                        ...
                                        {category?.length > 0 ?
                                            category?.map((item, i) => {
                                                if (item.createdAt) {
                                                    ...
                                                    return <div className="flex justify-center px-3 gap-2 bg-white rounded-md blur_effect items-center py-3" key={i}>
                                                        <p className="font-semibold">{item?.name}</p>
                                                        <MdCancel color="#374D99" className="cursor-pointer" onClick={() => handleRemoveCategory(item?._id)} />
                                                    </div>
                                                } else {
                                                    return null
                                                }
                                ...
                                {isVariableCondition ? (
                                    ...
                                    <div className="flex w-[100%] gap-3">
                                        ...
                                        <Field
                                            ...
                                            name={"discountPrice"}
                                            value={discountPrice}
                                            onChange={(e) => setProductData({ ...productData, discountPrice: Number(e.target.value) })}
                                        />
                                    </div>
                                )}
