...
const Field = ({ type = "text", disabled, placeHolder, name, ref, bgColor, value, className, onChange, xl, lg, md, isIcon, row, col, maxLength, StyleColor = "black", styles, customClass, pattern }) => {
   ...
   const inputProps = {
      ...
      onChange: onChange,
      pattern: pattern,
      className: className ? `${inputClassName} ${className}` : type === "password" || isIcon ? `w-full ${inputClassName}` : disabled ? `text-gray-500 cursor-not-allowed ${inputClassName}` : customClass ? customClass : `${inputClassName}`,
   };
   ...
   const textareaProps = {
      ...
      disabled: disabled,
      style: { background: "#F0F0F0", color: StyleColor },
      onChange: onChange,
      rows: row,
      cols: col,
      name: name,
